    ...
    def __init__(self, name, ob_space, ac_space, take_weights_here=None):
        ...
        with tf.variable_scope(name):
            obs_tuple = [
                ...
                tf.placeholder(tf.float32,        (None, 16), name="obs1"),
            ]
            ...
            norm_mean = tf.get_variable("norm_mean", [16])
            norm_std = tf.get_variable("norm_std", [16])
            x = tf.divide(x - norm_mean, norm_std)
            dense1_w = tf.get_variable("dense1_w", [16,64])
            dense1_b = tf.get_variable("dense1_b", [64])
            x = tf.matmul(x, dense1_w) + dense1_b
            x = selu(x)
            dense2_w = tf.get_variable("dense2_w", [64,64])
            dense2_b = tf.get_variable("dense2_b", [64])
            x = tf.matmul(x, dense2_w) + dense2_b
            x = selu(x)
            final_w = tf.get_variable("final_w", [64,4])
            final_b = tf.get_variable("final_b", [4])
        ...
        self.assigns = [
            (  norm_mean, take_weights_here["state_means"]),
            (  norm_std, take_weights_here["state_std"]),
            (  dense1_w, take_weights_here["w0"]),
            (  dense1_b, take_weights_here["b0"]),
            (  dense2_w, take_weights_here["w1"]),
            (  dense2_b, take_weights_here["b1"]),
            (   final_w, take_weights_here["w2"]),
            (   final_b, take_weights_here["b2"]),
        ]
...
def demo_run(video=False, monitor=False):
